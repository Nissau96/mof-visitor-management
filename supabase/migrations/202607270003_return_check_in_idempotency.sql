...
alter table public.used_visitor_verification_tokens
  add constraint used_visitor_tokens_visit_id_fkey
  foreign key (visit_id)
  references public.visits(id)
  on delete restrict;

create unique index used_visitor_tokens_visit_id_key
  on public.used_visitor_verification_tokens (visit_id)
...
begin
  ...
  -- If this exact token already completed a check-in,
  -- return the original result instead of creating another visit.
  select
    visit.id,
  ...
  -- A token record without a matching visit must not be reused.
  if exists (
    select 1
    from public.used_visitor_verification_tokens
      as used_token
