...
-- ============================================================
-- Phase 1: Tower-aware reception dashboard
-- ============================================================
--
-- Receptionists receive dashboard statistics and active visits
-- only for their selected tower.
--
-- Administrators may request Tower 1, Tower 2 or all towers.
--
-- The original five-parameter dashboard function remains
-- temporarily available until the updated API is deployed.
-- ============================================================
...
begin
  ...
  then
    raise exception using
      errcode = '22023',
      message =
        'Dashboard agency filter is too long';
  end if;
  ...
  then
    raise exception using
      errcode = '22023',
      message =
        'Dashboard division filter is too long';
  end if;
  ...
  -- Statistics use the same tower scope as the active-visitor
  -- records. A null scope is available only to administrators.
  select
    count(*) filter (
...
is
  'Returns tower-authorized dashboard statistics and paginated active visits to trusted server-side application functions.';
